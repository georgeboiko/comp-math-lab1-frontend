...
function Chord() {
    ...
    const handleSubmit = async () => {
        ...
        try {
            const resp = await fetch("/api/lab/2/chord", {
                method: "POST",
    ...
    if (eqLoading) return <div className="chord-status">[ LOADING EQUATIONS... ]</div>
    if (eqError)   return <div className="chord-status chord-status--error">!! ERROR: {eqError}</div>

    return (
        <div className="chord-wrapper">
            <div className="chord-section-title">--- МЕТОД ХОРД ---</div>
            <div className="chord-layout">
                <div className="chord-left">
                    <p>[EQUATION]:</p>
                    {equations.map((eq, idx) => {
                        const eqId = eq.equationId ?? idx
                        const latex = katex.renderToString((eq.equationLatex ?? eq.equationString) + " = 0", { throwOnError: false })
                        return (
                            <label key={eqId} className="chord-eq-option">
                                <input type="radio" name="chord-eq"
                    ...
                    {validationError && <div className="chord-validation-error">!! {validationError}</div>}
                    ...
                    {serverError && (
                        <div className="chord-output status-error">
                    ...
                    {result && (
                        <div className={`chord-output ${result.lab2IsSuccess ? "status-success" : "status-error"}`}>
                ...
                <div className="chord-right">
                    <p>[GRAPH]:</p>
                    <div ref={plotRef} className="chord-plot" />
                </div>
            </div>
        </div>
    )
